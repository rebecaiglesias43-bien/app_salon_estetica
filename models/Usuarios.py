...
class Usuarios(Model):
    ...
    def get_all(cls, limit=None, offset=None):
        sql = "SELECT usu_id, usu_username, usu_email, usu_estado FROM usuarios ORDER BY usu_username ASC"
    ...
    @classmethod
    def set_reset_token(cls, id, token, expires):
        sql = "UPDATE usuarios SET usu_reset_token = %s, usu_reset_expires = %s WHERE usu_id = %s"
        return cls.execute(sql, (token, expires, id))

    @classmethod
    def clear_reset_token(cls, id):
        sql = "UPDATE usuarios SET usu_reset_token = NULL, usu_reset_expires = NULL WHERE usu_id = %s"
        return cls.execute(sql, (id,))
