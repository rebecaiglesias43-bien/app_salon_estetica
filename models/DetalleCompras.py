...
class DetalleCompras(Model):
    ...
    def get_by_compra(cls, compra_id):
        sql = """
            ...
            JOIN productos p ON d.dco_producto_id = p.pro_id
    ...
    @classmethod
    def get_total_by_compra(cls, compra_id):
        sql = "SELECT COALESCE(SUM(dco_subtotal), 0) as total FROM detalle_compras WHERE dco_compra_id = %s"
        result = cls.query_one(sql, (compra_id,))
