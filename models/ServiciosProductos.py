...
class ServiciosProductos(Model):
    ...
    @classmethod
    def exists_duplicate(cls, servicio_id, producto_id, exclude_id=None):
        """Verifica si ya existe la asociación servicio-producto."""
        if exclude_id:
            sql = "SELECT COUNT(*) as cnt FROM servicios_productos WHERE sep_servicio_id = %s AND sep_producto_id = %s AND sep_id != %s"
            params = (servicio_id, producto_id, exclude_id)
        else:
            sql = "SELECT COUNT(*) as cnt FROM servicios_productos WHERE sep_servicio_id = %s AND sep_producto_id = %s"
